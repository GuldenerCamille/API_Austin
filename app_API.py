...
import joblib
import json
from utils import *  # Import custom utility functions if available
...
# Define column layout for better organization
...
with col1:
    ...
    with st.container(border=True):
        header("Animal adoption", 3)  # Custom header function to display titles
        header("Enter the animal's information :", 4)

        # Organize input fields in two columns
...
if st.button("Get Prediction"):
    ...
    response = requests.get(
        f'https://projete1-api.onrender.com/predict?Intake_Type={model_data_dict["Intake_Type"]}&Intake_Condition={model_data_dict["Intake_Condition"]}&Animal_Type={model_data_dict["Animal_Type"]}&Sex_upon_Intake={model_data_dict["Sex_upon_Intake"]}&Age_Upon_Intake_Days={model_data_dict["Age_Upon_Intake_Days"]}'
    )
    ...
    if response.status_code == 200:
        ...
        st.error(
            f"Failed to get prediction. Error code: {response.status_code}"
